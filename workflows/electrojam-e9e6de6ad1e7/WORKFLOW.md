...
- `pull`: keep branch updated with latest `orgiin/main` before handoff.
...
4. Check whether a PR already exists for the current branch and whether it is closed.
   ...
   - Create a fresh branch from `orgiin/main` and restart execution flow as a new attempt.
...
9. Run the `pull` skill to sync with latest `orgiin/main` before any code edits, then record the pull/sync result in the workpad `Notes`.
...
9. Merge latest `orgiin/main` into branch, resolve conflicts, and rerun checks.
...
5. Create a fresh branch from `orgiin/main`.
...
- For closed/merged branch PRs, create a new branch from `orgiin/main` and restart from reproduction/planning as if starting fresh.
